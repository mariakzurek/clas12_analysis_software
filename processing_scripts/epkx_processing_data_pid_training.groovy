    ...
    public static void main(String[] args) {
        ...
        for (int current_file = 0; current_file < n_files; current_file++) {
            ...
            while (reader.hasEvent()) {
                ...
                // ── Require FD proton tag for eKpX topology ─────────────────────
                boolean hasProtonTag = false

                for (int proton_row = 1; proton_row < rec_bank.rows(); proton_row++) {

                    if (rec_bank.getInt("pid", proton_row) != 2212)
                        continue

                    if (!passHadronCuts(proton_row, banks))
                        continue
